---
description: Update ZAgent from upstream when branch has diverged (ahead/behind)
---

# ZAgent Upstream Sync Workflow
...
# Move old version
mv /Applications/ZAgent.app /tmp/ZAgent-backup.app

# Install new build
cp -R dist/ZAgent.app /Applications/

# Launch
open /Applications/ZAgent.app
```
...
ps aux | grep -E "(zagent|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm zagent agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update zagent.json with fallback chains
...
pnpm zagent agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
