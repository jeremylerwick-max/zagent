...
def upgrade() -> None:
    # Extension required for Vector type
    ...
    op.create_table(
        ...
        sa.Column("embedding", sa.text("vector(1536)"), nullable=False),
    )
    ...
    # ivfflat index for cosine distance (tune lists for your scale)
    op.execute(
        """
...
def downgrade() -> None:
    ...
    # do not drop extension automatically (often shared)
