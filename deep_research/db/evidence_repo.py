...
from .models import (
    ...
    VerificationFinding as VFModel, FinalReport, ClaimCitationLink,
)
...
class EvidenceRepo:
    ...
    async def citation_enforcement_query(self, job_id: str, *, min_conf: float = 0.60) -> List[Tuple[int, str]]:
        q = sa.text("""
            ...
            GROUP BY sc.claim_id, sc.claim_text HAVING COUNT(ccl.citation_id) = 0
            ORDER BY sc.claim_id;
        """)
    ...
    async def finalize(self, job_id: str, answer: str, outline: Optional[str],
        ...
        stmt = (
            insert(FinalReport).values(
                job_id=job_id, answer=answer, outline=outline,
                confidence=confidence, used_source_ids=used_source_ids,
            ).on_conflict_do_update(
                index_elements=["job_id"],
                set_={"answer": sa.text("EXCLUDED.answer"), "outline": sa.text("EXCLUDED.outline"),
                       "confidence": sa.text("EXCLUDED.confidence"),
                       "used_source_ids": sa.text("EXCLUDED.used_source_ids")},
            )
        )
        await self.session.execute(stmt)
        await self.session.execute(
