...
This document describes how the ZAgent ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`zagent acp` exposes an ACP agent over stdio and forwards prompts to a running
ZAgent Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a ZAgent Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `zagent acp` over stdio.

Example config:

```bash
zagent config set gateway.remote.url wss://gateway-host:18789
zagent config set gateway.remote.token <token>
```

Example run:

```bash
zagent acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
zagent acp --session agent:main:main
zagent acp --session agent:design:main
zagent acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “ZAgent ACP” to start a thread.

## Execution Model

- ACP client spawns `zagent acp` and speaks ACP messages over stdio.
...
```bash
zagent acp --session agent:main:main
zagent acp --session-label "support inbox"
zagent acp --reset-session
```
...
`zagent acp` resolves the Gateway URL and auth from CLI flags or config:
