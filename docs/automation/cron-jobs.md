...
- Jobs persist under `~/.zagent/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.zagent/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.zagent/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
zagent cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
zagent cron edit <jobId> --agent ops
zagent cron edit <jobId> --clear-agent
```
```
...
```bash
zagent system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`zagent system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `ZAGENT_SKIP_CRON`.
