...
Matrix is an open, decentralized messaging protocol. ZAgent connects as a Matrix **user**
...
```bash
zagent plugins install @zagent/matrix
```

Local checkout (when running from a git repo):

```bash
zagent plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `zagent plugins install @zagent/matrix`
   - From a local checkout: `zagent plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: ZAgent calls the same
     login endpoint, stores the access token in `~/.zagent/credentials/matrix/credentials.json`,
...
- On first connection, ZAgent requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  ZAgent logs a warning.
...
`~/.zagent/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `zagent pairing list matrix`
  - `zagent pairing approve matrix <CODE>`
...
- On startup, ZAgent resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
