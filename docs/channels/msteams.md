...
```bash
zagent plugins install @zagent/msteams
```

Local checkout (when running from a git repo):
```bash
zagent plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
ZAgent will offer the local install path automatically.
...
3) Configure ZAgent with those credentials.
...
## Goals
- Talk to ZAgent via Teams DMs, group chats, or channels.
...
- On startup, ZAgent resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.zagent/zagent.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring ZAgent, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `zagent-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `zagent plugins install @zagent/msteams`
   - From a local checkout: `zagent plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "ZAgent in Teams", "full": "ZAgent in Teams" },
...
ZAgent handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, ZAgent only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ZAgentShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
ZAgent sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `zagent message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.zagent/msteams-polls.json`.
...
**CLI:**
```bash
zagent message send --channel msteams \
...
zagent message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
zagent message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
zagent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
zagent message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
