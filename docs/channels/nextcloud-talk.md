...
```bash
zagent plugins install @zagent/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
zagent plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "ZAgent" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `zagent pairing list nextcloud-talk`
  - `zagent pairing approve nextcloud-talk <CODE>`
