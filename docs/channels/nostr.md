...
read_when:
  - You want ZAgent to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables ZAgent to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`zagent onboard`) and `zagent channels add` list optional channel plugins.
...
```bash
zagent plugins install @zagent/nostr
```

Use a local checkout (dev workflows):

```bash
zagent plugins install --link <path-to-zagent>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "zagent@example.com",
        "lud16": "zagent@example.com"
      }
    }
  }
}
