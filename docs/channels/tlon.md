...
Tlon is a decentralized messenger built on Urbit. ZAgent connects to your Urbit ship and can
...
```bash
zagent plugins install @zagent/tlon
```

Local checkout (when running from a git repo):

```bash
zagent plugins install ./extensions/tlon
```
...
Use these with `zagent message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, ZAgent replies in-thread.
