...
read_when:
  - Setting up Twitch chat integration for ZAgent
---
# Twitch (plugin)

Twitch chat support via IRC connection. ZAgent connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
zagent plugins install @zagent/twitch
```

Local checkout (when running from a git repo):

```bash
zagent plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `ZAGENT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "zagent",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use ZAGENT_TWITCH_ACCESS_TOKEN env var)
...
```bash
ZAGENT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
