...
- **Session store** (chat history + routing state) under `~/.zagent/agents/<agentId>/sessions`.
...
available from `~/.zagent/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.zagent` (or `ZAGENT_STATE_DIR`)
- Workspace: `~/clawd` (or `~/clawd-<agentId>`)
- Agent dir: `~/.zagent/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.zagent/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, ZAgent runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/clawd` (or `~/clawd-<profile>` when `ZAGENT_PROFILE` is set).
- State defaults to `~/.zagent/agents/main/agent`.
...
```js
{
    ...
    list: [
      {
        ...
        agentDir: "~/.zagent/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/clawd-work",
        agentDir: "~/.zagent/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.zagent/credentials/whatsapp/personal
          // authDir: "~/.zagent/credentials/whatsapp/personal",
        },
        biz: {
