...
- `/usage cost` in chats: local cost summary aggregated from ZAgent session logs.
- CLI: `zagent status --usage` prints a full per-provider breakdown.
- CLI: `zagent channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
