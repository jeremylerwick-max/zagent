...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in ZAgent
---

# Node + tsx "__name is not a function" crash

## Summary
Running ZAgent via Node with `tsx` fails at startup with:

```
[zagent] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `zagent status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch zagent.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node zagent.mjs status` works on Node 25.
