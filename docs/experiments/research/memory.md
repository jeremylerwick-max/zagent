...
read_when:
  ...
  - Deciding: standalone CLI vs deep ZAgent integration
...
Recommendation: **deep integration in ZAgent**, but keep a separable core library.

### Why integrate into ZAgent?
- ZAgent already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `zagent memory recall "…" --k 25 --since 30d`
  - `zagent memory reflect --since 7d`
