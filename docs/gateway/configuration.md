---
summary: "All configuration options for ~/.zagent/zagent.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

ZAgent reads an optional **JSON5** config from `~/.zagent/zagent.json` (comments + trailing commas allowed).

If the file is missing, ZAgent uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
ZAgent only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `zagent doctor`, `zagent logs`, `zagent health`, `zagent status`, `zagent service`, `zagent help`).
- Run `zagent doctor` to see the exact issues.
- Run `zagent doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `zagent config set`. Keep a backup of `~/.zagent/zagent.json`.
...
```bash
zagent gateway call config.get --params '{}' # capture payload.hash
zagent gateway call config.apply --params '{
...
```bash
zagent gateway call config.get --params '{}' # capture payload.hash
zagent gateway call config.patch --params '{
...
{ "$include": "/etc/zagent/base.json5" } // absolute
...
```json5
// ~/.zagent/clients/mueller/agents.json5
[
...
```json5
// ~/.zagent/clients/mueller/broadcast.json5
{
...
ZAgent reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, ZAgent runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `ZAGENT_LOAD_SHELL_ENV=1`
- `ZAGENT_SHELL_ENV_TIMEOUT_MS=15000`
...
ZAgent stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.zagent/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.zagent/credentials/oauth.json` (or `$ZAGENT_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.zagent/agent/*` (migrated by `zagent doctor` into `~/.zagent/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `ZAGENT_OAUTH_DIR`
- Agent dir (default agent root override): `ZAGENT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, ZAgent imports `oauth.json` entries into `auth-profiles.json`.
...
If set, ZAgent derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/zagent/zagent-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/zagent/zagent.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.zagent/credentials/whatsapp/biz
          // authDir: "~/.zagent/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `zagent doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "zagent", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.zagent/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `zagent.json`).
...
ZAgent starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
ZAgent starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `zagent plugins install @zagent/mattermost` (or `./extensions/mattermost` from a git checkout).
...
ZAgent starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
ZAgent spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, ZAgent
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/zagent" } }
}
```
...
When a file exceeds this limit, ZAgent logs a warning and injects a truncated
...
message envelopes). If unset, ZAgent uses the host timezone at runtime.
...
`[zagent]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[zagent]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[zagent]"`, ZAgent will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, ZAgent generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.zagent/settings/tts.json",
      elevenlabs: {
...
ZAgent also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, ZAgent currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `zagent configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:zagent`: all built-in ZAgent tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.zagent/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.zagent/sandboxes",
        docker: {
          image: "zagent-sandbox:bookworm-slim",
          containerPrefix: "zagent-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "zagent-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "zagent-sandbox-browser:bookworm-slim",
          containerPrefix: "zagent-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
ZAgent uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.zagent/agents/<agentId>/agent/models.json` or by defining the same schema inside your
ZAgent config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, ZAgent writes/merges a `models.json` into
`~/.zagent/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. ZAgent uses
...
- Shortcut: `zagent onboard --auth-choice opencode-zen`.
...
Shortcut: `zagent onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `zagent onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `zagent onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `zagent onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `zagent onboard --auth-choice minimax-api`.
...
- Override the agent config root with `ZAGENT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.zagent/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.zagent/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.zagent/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, ZAgent stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.zagent/skills` (workspace skills
...
from `~/.zagent/extensions`, `<workspace>/.zagent/extensions`, plus any
...
ZAgent can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (ZAgent.app menubar, or `zagent gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/zagent" }
...
- Examples: `"/ui"`, `"/zagent"`, `"/apps/zagent"`.
...
- When a connection comes from one of these IPs, ZAgent uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `zagent gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `ZAGENT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `ZAGENT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. ZAgent
...
macOS app behavior:
- ZAgent.app watches `~/.zagent/zagent.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.zagent/zagent.json` (or `ZAGENT_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.zagent/zagent.json` (or `ZAGENT_CONFIG_PATH`)
...
- `ZAGENT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `zagent --dev …` → uses `~/.zagent-dev` + shifts ports from base `19001`
- `zagent --profile <name> …` → uses `~/.zagent-<name>` (port via config/env/flags)
...
Example:
```bash
ZAGENT_CONFIG_PATH=~/.zagent/a.json \
ZAGENT_STATE_DIR=~/.zagent-a \
zagent gateway --port 19001
```
...
Gmail helper config (used by `zagent webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "zagent@gmail.com",
...
- Set `ZAGENT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, ZAgent defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__zagent/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__zagent__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.zagent/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.zagent/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.zagent/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_zagent-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_zagent-bridge._tcp` under `~/.zagent/dns/` using the standard discovery domain `zagent.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `zagent.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `zagent.internal` via that server
