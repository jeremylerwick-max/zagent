...
- During heartbeat runs, ZAgent treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `zagent sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, ZAgent skips the heartbeat run entirely (no model call).
...
```bash
zagent system event --text "Check for urgent follow-ups" --mode now
```
