...
- Replaces the legacy `gateway` command. CLI entry point: `zagent gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
zagent gateway --port 18789
# for full debug/trace logs in stdio:
zagent gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
zagent gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.zagent/zagent.json` (or `ZAGENT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__zagent__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `ZAGENT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `ZAGENT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `ZAGENT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.zagent.*` may still exist)
- Linux: `zagent-gateway-<profile>.service`
- Windows: `ZAgent Gateway (<profile>)`

Install metadata is embedded in the service config:
- `ZAGENT_SERVICE_MARKER=zagent`
- `ZAGENT_SERVICE_KIND=gateway`
- `ZAGENT_SERVICE_VERSION=<version>`
...
```bash
zagent --dev setup
zagent --dev gateway --allow-unconfigured
...
- `ZAGENT_STATE_DIR=~/.zagent-dev`
- `ZAGENT_CONFIG_PATH=~/.zagent-dev/zagent.json`
- `ZAGENT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- Base port = `gateway.port` (or `ZAGENT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `ZAGENT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `ZAGENT_CONFIG_PATH`
- unique `ZAGENT_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
zagent --profile main gateway install
zagent --profile rescue gateway install
```

Example:
```bash
ZAGENT_CONFIG_PATH=~/.zagent/a.json ZAGENT_STATE_DIR=~/.zagent-a zagent gateway --port 19001
ZAGENT_CONFIG_PATH=~/.zagent/b.json ZAGENT_STATE_DIR=~/.zagent-b zagent gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `zagent health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `zagent`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `zagent gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.zagent.*` is cleaned up).
  - `zagent doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are ZAgent profile services.
  ...
  - Cleanup: `zagent gateway uninstall` (current service) and `zagent doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `zagent gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- ZAgent.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.zagent.*` labels still unload cleanly).
- To stop it cleanly, use `zagent gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `zagent gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `zagent gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
ZAgent installs a **systemd user service** by default on Linux/WSL2. We
...
`zagent gateway install` writes the user unit. `zagent doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/zagent-gateway[-<profile>].service`:
```
[Unit]
Description=ZAgent Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/zagent gateway --port 18789
Restart=always
RestartSec=5
Environment=ZAGENT_GATEWAY_TOKEN=
...
```
systemctl --user enable --now zagent-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/zagent-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now zagent-gateway[-<profile>].service
```
...
## CLI helpers
- `zagent gateway health|status` — request health/status over the Gateway WS.
- `zagent message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `zagent agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `zagent gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `zagent gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `zagent gateway` and the legacy TCP control port.
