---
summary: "Quick troubleshooting guide for common ZAgent failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When ZAgent misbehaves, here's how to fix it.
...
|---|---|---|
| `zagent status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `zagent status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `zagent status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `zagent gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `zagent channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `zagent gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `zagent logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `zagent status --all` (it redacts tokens). If you paste `zagent status`, consider setting `ZAGENT_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  zagent models auth setup-token --provider anthropic
  ```
...
```bash
zagent models auth paste-token --provider anthropic
zagent models status
```
...
**Logs:**
- Preferred: `zagent logs --follow`
- File logs (always): `/tmp/zagent/zagent-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$ZAGENT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u zagent-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  zagent config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  zagent config set gateway.mode remote
  zagent config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `zagent setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.zagent/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `zagent doctor`
...
- then run `zagent sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `zagent gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `zagent gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `zagent gateway status` and `zagent doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `ZAGENT_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `zagent gateway status` shows a config mismatch**
...
- Fix: rerun `zagent gateway install --force` from the same `--profile` / `ZAGENT_STATE_DIR` you want the service to use.

**If `zagent gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `zagent doctor` to update it (or `zagent gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `ZAGENT_GATEWAY_TOKEN`) and restart the service.

**If `zagent gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/zagent` on disk.
...
Symptoms: `pwd` or file tools show `~/.zagent/sandboxes/...` even though you
...
ZAgent intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `zagent models list` or
  `zagent models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${ZAGENT_CONFIG_PATH:-$HOME/.zagent/zagent.json}"
```
...
tail -f "$(ls -t /tmp/zagent/zagent-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
zagent logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.zagent/agents/<agentId>/sessions/
```
...
zagent logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
zagent gateway --verbose
```

If you’re logged out / unlinked:

```bash
zagent channels logout
trash "${ZAGENT_STATE_DIR:-$HOME/.zagent}/credentials" # if logout can't cleanly remove everything
zagent channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/zagent/zagent-*.log | head -1)" | tail -20
```

### High Memory Usage

ZAgent keeps conversation history in memory.
...
ZAgent now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `zagent doctor` reports every invalid entry.
- `zagent doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `zagent logs`, `zagent health`, `zagent status`, `zagent gateway status`, and `zagent gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/zagent/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `zagent models status` (CLI).
...
5) Run `zagent channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. ZAgent scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/zagent/zagent/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
zagent gateway status
zagent gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.zagent.* still works)
```
...
Ensure the global `zagent` CLI is installed and matches the app version:
```bash
zagent --version
npm install -g zagent@<version>
```
...
#   ${ZAGENT_CONFIG_PATH:-$HOME/.zagent/zagent.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
zagent gateway --verbose
zagent channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/zagent/zagent-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$ZAGENT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.zagent/logs/...`; profiles use `~/.zagent-<profile>/logs/...`)<br />Linux: `journalctl --user -u zagent-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "ZAgent Gateway (<profile>)" /V /FO LIST` |
| Session files | `$ZAGENT_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$ZAGENT_STATE_DIR/media/` |
| Credentials | `$ZAGENT_STATE_DIR/credentials/` |
...
zagent gateway status --deep

# Is the gateway reachable?
zagent health --json
# If it fails, rerun with connection details:
zagent health --verbose
...
# zagent gateway uninstall

trash "${ZAGENT_STATE_DIR:-$HOME/.zagent}"
zagent channels login         # re-pair WhatsApp
zagent gateway restart           # or: zagent gateway
```
...
1. Check logs first: `/tmp/zagent/` (default: `zagent-YYYY-MM-DD.log`, or your configured `logging.file`)
