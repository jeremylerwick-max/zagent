...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `zagent sandbox explain`.
...
Legacy `agent.*` configs are migrated by `zagent doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=zagent.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${ZAGENT_STATE_DIR:-$HOME/.zagent}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
