...
service via the external `zagent` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`ZAGENT_PROFILE`; legacy `com.zagent.*` is supported).
