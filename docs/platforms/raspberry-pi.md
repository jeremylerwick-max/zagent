---
summary: "ZAgent on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up ZAgent on a Raspberry Pi
  - Running ZAgent on ARM devices
  - Building a cheap always-on personal AI
---

# ZAgent on Raspberry Pi

## Goal

Run a persistent, always-on ZAgent Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most ZAgent features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled zagent
...
```bash
# Check logs
journalctl -u zagent --no-pager -n 100

# Common fix: rebuild
cd ~/zagent  # if using hackable install
npm run build
sudo systemctl restart zagent
```
