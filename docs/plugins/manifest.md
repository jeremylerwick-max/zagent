...
read_when:
  - You are building a ZAgent plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (zagent.plugin.json)

Every plugin **must** ship a `zagent.plugin.json` file in the **plugin root**.
ZAgent uses this manifest to validate configuration **without executing plugin
