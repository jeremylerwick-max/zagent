---
summary: "Model providers (LLMs) supported by ZAgent"
read_when:
...
ZAgent can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `zagent onboard`).
