---
summary: "Use Synthetic's Anthropic-compatible API in ZAgent"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. ZAgent registers it as the
...
```bash
zagent onboard --auth-choice synthetic-api-key
```
...
Note: ZAgent's Anthropic client appends `/v1` to the base URL, so use
