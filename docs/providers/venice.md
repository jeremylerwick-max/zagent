---
summary: "Use Venice AI privacy-focused models in ZAgent"
read_when:
  - You want privacy-focused inference in ZAgent
...
```bash
zagent onboard --auth-choice venice-api-key
```
...
```bash
zagent chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, ZAgent shows all available Venice models. Pick based on your needs:
...
```bash
zagent models set venice/claude-opus-45
zagent models set venice/llama-3.3-70b
```

List all available models:

```bash
zagent models list | grep venice
```

## Configure via `zagent configure`

1. Run `zagent configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
ZAgent automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
zagent chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
zagent chat --model venice/claude-opus-45

# Use uncensored model
zagent chat --model venice/venice-uncensored

# Use vision model with image
zagent chat --model venice/qwen3-vl-235b-a22b

# Use coding model
zagent chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `zagent models list` to see currently available models. Some models may be temporarily offline.
