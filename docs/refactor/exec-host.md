...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.zagent/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.zagent/node.json` (node id + pairing token).
...
Path: `~/.zagent/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.zagent/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.zagent/exec-approvals.sock` (0600).
