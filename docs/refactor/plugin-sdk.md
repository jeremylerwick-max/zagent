...
Delivery:
- Publish as `@zagent/plugin-sdk` (or export from core under `zagent/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `ZAgentPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ZAgentConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ZAgentConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ZAgentConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: ZAgentConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ZAgentConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ZAgentConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@zagent/plugin-sdk`.
- Add `api.runtime` to `ZAgentPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `zagentRuntime: ">=2026.2.0"`).
