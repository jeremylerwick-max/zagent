...
ZAgent features that can generate provider usage or paid API calls.
...
- `zagent status --usage` and `zagent channels list` show provider **usage windows**
...
ZAgent can pick up credentials from:
...
### 8) Model scan / probe
`zagent models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
