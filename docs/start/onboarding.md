---
summary: "First-run onboarding flow for ZAgent (macOS app)"
read_when:
...
- Writes credentials to `~/.zagent/credentials/oauth.json`
...
The app can install the global `zagent` CLI via npm/pnpm so terminal
...
On the first agent run, ZAgent bootstraps a workspace (default `~/clawd`):
...
```bash
zagent webhooks gmail setup --account you@gmail.com
```
