...
The onboarding wizard is the **recommended** way to set up ZAgent on macOS,
...
`zagent dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
...
(`web_fetch` works without a key). Easiest path: `zagent configure --section web`
...
1) **Existing config detection**
   - If `~/.zagent/zagent.json` exists, choose **Keep / Modify / Reset**.
   - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
     (or pass `--reset`).
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `zagent doctor` before continuing.
...
2) **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.zagent/.env` so launchd can read it.
  ...
  - OAuth credentials live in `~/.zagent/credentials/oauth.json`; auth profiles live in `~/.zagent/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `zagent pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `zagent health`.
   - Tip: `zagent status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `zagent agents add <name>` to create a separate agent with its own workspace,
...
```bash
zagent onboard --non-interactive \
  --mode local \
  --auth-choice apiKey \
...
- Stores it under `~/.zagent/tools/signal-cli/<version>/`.
...
Typical fields in `~/.zagent/zagent.json`:
...
`zagent agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.zagent/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.zagent/agents/<agentId>/sessions/`.
