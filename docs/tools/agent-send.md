---
summary: "Direct `zagent agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `zagent agent` (direct agent runs)

`zagent agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `zagent message --target`).
...
```bash
zagent agent --to +15555550123 --message "status update"
...
zagent agent --to +15555550123 --message "Summon reply" --deliver
