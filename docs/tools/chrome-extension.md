---
summary: "Chrome extension: let ZAgent drive your existing Chrome tab"
read_when:
...
The ZAgent Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawd-managed Chrome profile.
...
ZAgent then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the ZAgent release (npm package) as static files. There is no separate “build” step.

After upgrading ZAgent:
- Re-run `zagent browser extension install` to refresh the installed files under your ZAgent state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

ZAgent ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want ZAgent to control.
...
- `ON`: attached; ZAgent can drive that tab.
...
Debugging: `zagent sandbox explain`
...
`zagent browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `zagent browser extension install` first to copy the extension to a stable location under your ZAgent state directory.
