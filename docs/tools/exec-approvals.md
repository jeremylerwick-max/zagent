...
- **gateway host** → `zagent` process on the gateway machine
...
`~/.zagent/exec-approvals.json` directly.

CLI: `zagent approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
