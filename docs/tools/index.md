---
summary: "Agent tool surface for ZAgent (browser, canvas, nodes, message, cron) replacing legacy `zagent-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `zagent-*` skills
---

# Tools (ZAgent)

ZAgent exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `zagent-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `zagent.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, ZAgent logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`zagent node run`).
...
Notes:
- Requires a Brave API key (recommended: `zagent configure --section web`, or set `BRAVE_API_KEY`).
...
- Quick smoke: `zagent nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `zagent gateway` restart in-place)
