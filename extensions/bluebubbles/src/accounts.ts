...
export function listBlueBubblesAccountIds(cfg: ZAgentConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: ZAgentConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: ZAgentConfig): ResolvedBlueBubblesAccount[] {
