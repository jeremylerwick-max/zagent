...
export function listGoogleChatAccountIds(cfg: ZAgentConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: ZAgentConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: ZAgentConfig): ResolvedGoogleChatAccount[] {
