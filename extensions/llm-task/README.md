...
`clawd.invoke --each`) without adding new ZAgent code per workflow.
...
This extension depends on ZAgent internal modules (the embedded agent runner).
It is intended to ship as a **bundled** ZAgent extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.zagent/extensions` as a standalone plugin directory.
