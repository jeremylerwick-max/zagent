...
// NOTE: This extension is intended to be bundled with ZAgent.
// When running from source (tests/dev), ZAgent internals live under src/.
...
export function createLlmTaskTool(api: ZAgentPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-llm-task-"));
