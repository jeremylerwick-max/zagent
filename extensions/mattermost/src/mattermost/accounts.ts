...
export function listMattermostAccountIds(cfg: ZAgentConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: ZAgentConfig): string {
...
export function listEnabledMattermostAccounts(cfg: ZAgentConfig): ResolvedMattermostAccount[] {
