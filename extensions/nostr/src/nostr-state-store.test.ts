...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.ZAGENT_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-nostr-"));
  process.env.ZAGENT_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.ZAGENT_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".zagent");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.ZAGENT_STATE_DIR;
    else process.env.ZAGENT_STATE_DIR = previous;
