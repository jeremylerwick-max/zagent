...
metadata: {"zagent":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == ZAgent `sessions_spawn`
- **File I/O** == ZAgent `read`/`write`
- **Remote fetch** == ZAgent `web_fetch` (or `exec` with curl when POST is required)
