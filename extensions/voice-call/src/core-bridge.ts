...
function resolveZAgentRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.ZAGENT_ROOT?.trim() || process.env.ZAGENT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["zagent", "zagent"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set ZAGENT_ROOT (or legacy ZAGENT_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveZAgentRoot();
