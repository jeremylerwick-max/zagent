import type { ZAgentConfig } from "zagent/plugin-sdk";
...
export function listZaloAccountIds(cfg: ZAgentConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: ZAgentConfig): string {
...
function mergeZaloAccountConfig(cfg: ZAgentConfig, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: ZAgentConfig): ResolvedZaloAccount[] {
