...
ZAgent extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
zagent channels login --channel zalouser
...
```bash
zagent message send --channel zalouser --target <threadId> --message "Hello from ZAgent!"
```
...
```bash
zagent channels login --channel zalouser              # Login via QR
zagent channels login --channel zalouser --account work
zagent channels status --probe
zagent channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
zagent directory self --channel zalouser
zagent directory peers list --channel zalouser --query "name"
zagent directory groups list --channel zalouser --query "work"
zagent directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
zagent message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
zagent message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `zagent channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
zagent channels login --channel zalouser --account work
zagent message send --channel zalouser --account work --target <id> --message "Hello"
