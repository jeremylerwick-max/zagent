...
#
# Suggested cron: */30 * * * * /home/admin/zagent/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send ZAgent notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., zagent-alerts)
...
STATE_FILE="$HOME/.zagent/auth-monitor-state"
...
send_notification() {
    ...
    # Send via ZAgent if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use zagent
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via ZAgent to $NOTIFY_PHONE..."
            zagent send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! ZAgent is down. Run: ssh l36 '~/zagent/scripts/mobile-reauth.sh'" "urgent"
    exit 1
