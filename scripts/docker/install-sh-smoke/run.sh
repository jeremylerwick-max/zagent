...
SMOKE_PREVIOUS_VERSION="${ZAGENT_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${ZAGENT_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="zagent"
if [[ -z "${ZAGENT_INSTALL_PACKAGE:-}" && "$INSTALL_URL" == *"clawd.bot"* ]]; then
  DEFAULT_PACKAGE="zagent"
fi
PACKAGE_NAME="${ZAGENT_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
if [[ "$PACKAGE_NAME" == "zagent" ]]; then
  ALT_PACKAGE_NAME="zagent"
else
  ALT_PACKAGE_NAME="zagent"
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (ZAGENT_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${ZAGENT_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "$ZAGENT_INSTALL_LATEST_OUT"
fi
