...
IMAGE_NAME="zagent-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/zagent-bin:$PATH"
  mkdir -p /tmp/zagent-bin

  cat > /tmp/zagent-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/zagent-bin/systemctl

  cat > /tmp/zagent-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/zagent-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/zagent"
  npm_entry="/tmp/npm-prefix/lib/node_modules/zagent/zagent.mjs"
  git_entry="/app/zagent.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/zagent-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/zagent-gateway.service"
    if [ ! -f "$unit_path" ]; then
