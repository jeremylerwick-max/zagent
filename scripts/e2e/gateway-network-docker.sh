...
IMAGE_NAME="zagent-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="zagent-net-e2e-$$"
GW_NAME="zagent-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "ZAGENT_GATEWAY_TOKEN=$TOKEN" \
	  -e "ZAGENT_SKIP_CHANNELS=1" \
	  -e "ZAGENT_SKIP_GMAIL_WATCHER=1" \
	  -e "ZAGENT_SKIP_CRON=1" \
	  -e "ZAGENT_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
