...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/zagent-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/zagent-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/zagent-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/zagent-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.zagent/zagent.json"
    sessions_dir="$HOME/.zagent/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.zagent/zagent.json" <<'"'"'JSON'"'"'
{
