...
ZIP=${1:?"Usage: $0 ZAgent-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/zagent/zagent/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^ZAgent-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/zagent/zagent/releases/download/v${VERSION}/"}
