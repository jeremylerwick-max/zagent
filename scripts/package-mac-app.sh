...
# Build and bundle ZAgent into a minimal .app we can open.
# Outputs to dist/ZAgent.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/ZAgent.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/zagent/zagent/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/ZAgent/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ZAgentBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ZAgentGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/ZAgent"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/ZAgent"
fi
chmod +x "$APP_ROOT/Contents/MacOS/ZAgent"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/ZAgent" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/ZAgent/Resources/ZAgent.icns" "$APP_ROOT/Contents/Resources/ZAgent.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/ZAgent/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ZAgentKit resources"
ZAGENTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ZAgentKit_ZAgentKit.bundle"
if [ -d "$ZAGENTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ZAgentKit_ZAgentKit.bundle"
  cp -R "$ZAGENTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ZAgentKit_ZAgentKit.bundle"
else
  echo "WARN: ZAgentKit resource bundle not found at $ZAGENTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running ZAgent"
killall -q ZAgent 2>/dev/null || true
