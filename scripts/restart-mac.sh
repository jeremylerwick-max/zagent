#!/usr/bin/env bash
# Reset ZAgent like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ZAGENT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="ZAgent.app/Contents/MacOS/ZAgent"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/ZAgent"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/ZAgent"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/ZAgent"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/bot.molt.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/zagent-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${ZAGENT_RESTART_LOG:-/tmp/zagent-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ZAGENT_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.zagent/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ZAGENT_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node zagent.mjs daemon install --force --runtime node"
      log "  node zagent.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.zagent/disable-launchagent"
      log ""
...
log "==> Killing existing ZAgent instances"
kill_all_zagent
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product ZAgent"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/ZAgent.app" ]]; then
    APP_BUNDLE="/Applications/ZAgent.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/ZAgent.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/ZAgent.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/ZAgent.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set ZAGENT_APP_BUNDLE to your installed ZAgent.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node zagent.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node zagent.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".zagent", "zagent.json"), "utf8");
