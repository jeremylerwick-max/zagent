...
const compiler = env.ZAGENT_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.ZAGENT_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.ZAGENT_RUNNER_LOG === "0") return;
  process.stderr.write(`[zagent] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["zagent.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
