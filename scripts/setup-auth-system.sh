#!/bin/bash
# Setup ZAgent Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'zagent-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  ZAgent message: Send warning via ZAgent itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/zagent-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/zagent-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/zagent-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now zagent-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status zagent-auth-monitor.timer"
echo ""
echo "Quick commands:"
