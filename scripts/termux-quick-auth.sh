...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/zagent/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "ZAgent Re-Auth" -c "After getting key, run: ssh $SERVER '~/zagent/scripts/mobile-reauth.sh'" --id clawd-auth
        ;;
    *)
