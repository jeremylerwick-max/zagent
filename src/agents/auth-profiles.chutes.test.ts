...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.ZAGENT_AGENT_DIR;
    else process.env.ZAGENT_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-chutes-"));
    process.env.ZAGENT_STATE_DIR = tempDir;
    process.env.ZAGENT_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
