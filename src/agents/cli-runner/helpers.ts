...
/**
 * Cleanup suspended ZAgent CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: ZAgentConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-cli-images-"));
  const paths: string[] = [];
