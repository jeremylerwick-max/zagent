...
const defaultTools = createZAgentCodingTools();

describe("createZAgentCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createZAgentCodingTools();
    ...
    try {
      ...
      const contents = "Hello from zagent read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "zagent-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "zagent-workspace"),
    ...
    const tools = createZAgentCodingTools({ sandbox });
