...
import { __testing, createZAgentCodingTools } from "./pi-tools.js";
...
describe("createZAgentCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createZAgentCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "zagent-outside.txt");
