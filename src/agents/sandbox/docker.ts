...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "zagent.sandbox=1");
  args.push("--label", `zagent.sessionKey=${params.scopeKey}`);
  args.push("--label", `zagent.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `zagent.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "zagent.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`zagent sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`zagent sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("zagent sandbox recreate --all");
}
