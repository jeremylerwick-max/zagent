import type { ZAgentConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: ZAgentConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ZAgentConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: ZAgentConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ZAgentConfig) {
