...
import { scheduleGatewaySigusr1Restart, triggerZAgentRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting zagent in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerZAgentRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting zagent via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
