...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-commands-"));
...
function buildParams(commandBody: string, cfg: ZAgentConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
