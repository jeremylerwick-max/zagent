...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.zagentCanvasA2UIAction ||
        window.webkit.messageHandlers.zagentCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.zagentCanvasA2UIAction &&
        typeof window.zagentCanvasA2UIAction.postMessage === "function") ||
      (window.zagentCanvasA2UIAction &&
        typeof window.zagentCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.zagentSendUserAction === "function" ? window.zagentSendUserAction : undefined;
  if (!window.zagentSendUserAction && legacySend) {
    window.zagentSendUserAction = legacySend;
  }
  if (!window.zagentSendUserAction && typeof window.zagentSendUserAction === "function") {
    window.zagentSendUserAction = window.zagentSendUserAction;
  }
  const hasHelper = () =>
    typeof window.zagentSendUserAction === "function" ||
    typeof window.zagentSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("zagent:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ZAgent node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.zagentSendUserAction === "function"
        ? window.zagentSendUserAction
        : window.zagentSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ZAGENT_SKIP_CANVAS_HOST)) return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ZAgent Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
