...
import { discoverZAgentPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { ZAgentPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  zagent?: ZAgentPackageManifest;
  [LEGACY_MANIFEST_KEY]?: ZAgentPackageManifest;
};
...
const ENV_CATALOG_PATHS = ["ZAGENT_PLUGIN_CATALOG_PATHS", "ZAGENT_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<ZAgentPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: ZAgentPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageZAgent?: ZAgentPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageZAgent;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.zagent ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageZAgent: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverZAgentPlugins({ workspaceDir: options.workspaceDir });
