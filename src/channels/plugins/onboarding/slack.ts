...
function setSlackDmPolicy(cfg: ZAgentConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "ZAgent";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for ZAgent`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: ZAgentConfig, allowFrom: string[]): ZAgentConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
