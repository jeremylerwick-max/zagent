...
function setWhatsAppDmPolicy(cfg: ZAgentConfig, dmPolicy: DmPolicy): ZAgentConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: ZAgentConfig, allowFrom?: string[]): ZAgentConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: ZAgentConfig, selfChatMode: boolean): ZAgentConfig {
...
async function detectWhatsAppLinked(cfg: ZAgentConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so ZAgent can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for ZAgent" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("zagent channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
