...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zagent-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...
      ZAGENT_SKIP_GMAIL_WATCHER: "1",
      ZAGENT_SKIP_CRON: "1",
      ZAGENT_SKIP_BROWSER_CONTROL_SERVER: "1",
      ZAGENT_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "zagent-entry-bootstrap.mjs");
