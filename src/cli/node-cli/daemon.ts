...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("zagent node install"), formatCliCommand("zagent node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("zagent node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    ZAGENT_LOG_PREFIX: baseEnv.ZAGENT_LOG_PREFIX ?? "node",
