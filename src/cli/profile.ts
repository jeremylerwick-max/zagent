...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.zagent${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ZAGENT_PROFILE = profile;

  const stateDir = env.ZAGENT_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.ZAGENT_STATE_DIR?.trim()) env.ZAGENT_STATE_DIR = stateDir;

  if (!env.ZAGENT_CONFIG_PATH?.trim()) {
    env.ZAGENT_CONFIG_PATH = path.join(stateDir, "zagent.json");
  }

  if (profile === "dev" && !env.ZAGENT_GATEWAY_PORT?.trim()) {
    env.ZAGENT_GATEWAY_PORT = "19001";
  }
}
