...
import { resolveZAgentPackageRoot } from "../infra/zagent-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "zagent doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "zagent";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "zagent"]);
const CLI_NAME = resolveCliName();
const ZAGENT_REPO_URL = "https://github.com/zagent/zagent.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "zagent");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("zagent@")) return trimmed.slice("zagent@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.ZAGENT_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ZAGENT_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ZAGENT_GIT_DIR points at a non-git directory: ${params.dir}. Set ZAGENT_GIT_DIR to an empty folder or a zagent checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ZAGENT_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveZAgentPackageRoot({
  ...
  defaultRuntime.log(theme.heading("ZAgent update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating ZAgent..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this ZAgent install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("zagent doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("zagent gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g zagent@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g zagent@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.ZAGENT_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.ZAGENT_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("zagent gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("zagent doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("zagent gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("zagent gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `zagent update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `ZAGENT_GIT_DIR points at a non-git directory: ${gitDir}. Set ZAGENT_GIT_DIR to an empty folder or a zagent checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via ZAGENT_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update ZAgent to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["zagent update", "Update a source checkout (git)"],
        ["zagent update --channel beta", "Switch to beta channel (git + npm)"],
        ["zagent update --channel dev", "Switch to dev channel (git + npm)"],
        ["zagent update --tag beta", "One-off update to a dist-tag or version"],
        ["zagent update --no-restart", "Update without restarting the service"],
        ["zagent update --json", "Output result as JSON"],
        ["zagent update --yes", "Non-interactive (accept downgrade prompts)"],
        ["zagent update wizard", "Interactive update wizard"],
        ["zagent --update", "Shorthand for zagent update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run zagent update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.molt.bot/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["zagent update status", "Show channel + version status."],
          ["zagent update status --json", "JSON output."],
          ["zagent update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
