...
export function listAgentEntries(cfg: ZAgentConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ZAgentConfig, agentId: string) {
...
function resolveAgentModel(cfg: ZAgentConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ZAgentConfig): AgentSummary[] {
