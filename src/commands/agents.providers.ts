...
function resolveDefaultAccountId(cfg: ZAgentConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: ZAgentConfig): boolean {
...
export function summarizeBindings(cfg: ZAgentConfig, bindings: AgentBinding[]): string[] {
