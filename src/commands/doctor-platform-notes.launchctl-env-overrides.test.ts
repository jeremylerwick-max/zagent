...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ZAGENT_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ZAGENT_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ZAGENT_GATEWAY_TOKEN");
    expect(message).not.toContain("ZAGENT_GATEWAY_PASSWORD");
  });
