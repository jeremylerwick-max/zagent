...
function resolveMode(cfg: ZAgentConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveZAgentPackageRoot({
  ...
  let cfg: ZAgentConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("zagent configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("zagent config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("zagent setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("zagent doctor --fix")}" to apply changes.`);
  }
