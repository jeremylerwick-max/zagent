...
beforeEach(() => {
  ...
  resolveZAgentPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ZAGENT_STATE_DIR;
  originalUpdateInProgress = process.env.ZAGENT_UPDATE_IN_PROGRESS;
  process.env.ZAGENT_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "zagent-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.ZAGENT_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.ZAGENT_UPDATE_IN_PROGRESS;
  } else {
    process.env.ZAGENT_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveZAgentPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadZAgentPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/zagent-root.js", () => ({
  resolveZAgentPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "zagent-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.ZAGENT_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when ZAGENT_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/zagent.json",
      exists: true,
      raw: "{}",
    ...
    process.env.ZAGENT_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
