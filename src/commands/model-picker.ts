...
function resolveConfiguredModelRaw(cfg: ZAgentConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: ZAgentConfig): string[] {
...
export function applyPrimaryModel(cfg: ZAgentConfig, model: string): ZAgentConfig {
...
export function applyModelAllowlist(cfg: ZAgentConfig, models: string[]): ZAgentConfig {
  const defaults = cfg.agents?.defaults;
