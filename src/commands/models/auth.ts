...
import { readConfigFileSnapshot, type ZAgentConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: ZAgentConfig, model: string): ZAgentConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("zagent plugins install")}\`.`,
    );
  }
