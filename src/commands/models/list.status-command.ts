...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("zagent models auth setup-token")}\` or \`${formatCliCommand("zagent configure")}\`.`
          : `Run \`${formatCliCommand("zagent configure")}\` or set an API key env var.`;
