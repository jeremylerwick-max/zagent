...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveZAgentAgentDir: vi.fn().mockReturnValue("/tmp/zagent-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveZAgentAgentDir: mocks.resolveZAgentAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/zagent-agent/auth-profiles.json");
