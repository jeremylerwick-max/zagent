...
const requireAgentDir = () => {
  const agentDir = process.env.ZAGENT_AGENT_DIR;
  if (!agentDir) throw new Error("ZAGENT_AGENT_DIR not set");
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.ZAGENT_STATE_DIR;
  const previousAgentDir = process.env.ZAGENT_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.ZAGENT_STATE_DIR = previousStateDir;
    }
    if (previousAgentDir === undefined) {
      delete process.env.ZAGENT_AGENT_DIR;
    } else {
      process.env.ZAGENT_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.ZAGENT_OAUTH_DIR;
  });

  it("writes auth-profiles.json under ZAGENT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-oauth-"));
    process.env.ZAGENT_STATE_DIR = tempStateDir;
    process.env.ZAGENT_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ZAGENT_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to ZAGENT_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-minimax-"));
    process.env.ZAGENT_STATE_DIR = tempStateDir;
    process.env.ZAGENT_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.ZAGENT_AGENT_DIR;
