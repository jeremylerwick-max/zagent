...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.ZAGENT_GATEWAY_PASSWORD,
  };
  ...
  const initStateDir = async (prefix: string) => {
    ...
    process.env.ZAGENT_STATE_DIR = stateDir;
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.ZAGENT_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.ZAGENT_STATE_DIR = stateDir;
    process.env.ZAGENT_CONFIG_PATH = path.join(stateDir, "zagent.json");
