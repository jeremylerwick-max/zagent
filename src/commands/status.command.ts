...
export async function statusCommand(
  ...
  runtime.log(theme.heading("ZAgent status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("zagent security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("zagent security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("zagent status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("zagent logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("zagent status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("zagent gateway probe")}`);
  }
}
