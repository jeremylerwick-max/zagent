...
const ETC_ZAGENT_DIR = path.join(ROOT_DIR, "etc", "zagent");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "zagent.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcZAgentPath(...parts: string[]) {
  return path.join(ETC_ZAGENT_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcZAgentPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "zagent.json"))).toEqual({
      shared: true,
    });
  });
});
