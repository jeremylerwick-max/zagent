...
import type { ZAgentConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareZAgentVersions } from "./version.js";
...
function coerceConfig(value: unknown): ZAgentConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as ZAgentConfig;
}
...
function stampConfigVersion(cfg: ZAgentConfig): ZAgentConfig {
...
function warnIfConfigFromFuture(cfg: ZAgentConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareZAgentVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ZAgent (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: ZAgentConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ZAgentConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as ZAgentConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ZAgentConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ZAgentConfig) {
    clearConfigCache();
...
// module scope. `ZAGENT_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ZAGENT_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.ZAGENT_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): ZAgentConfig {
...
export async function writeConfigFile(cfg: ZAgentConfig): Promise<void> {
  clearConfigCache();
