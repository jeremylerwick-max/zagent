import type { ZAgentConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: ZAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: ZAgentConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: ZAgentConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: ZAgentConfig): boolean {
...
function isIMessageConfigured(cfg: ZAgentConfig): boolean {
...
function isWhatsAppConfigured(cfg: ZAgentConfig): boolean {
...
function isGenericChannelConfigured(cfg: ZAgentConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: ZAgentConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ZAgentConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: ZAgentConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: ZAgentConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: ZAgentConfig, pluginId: string): ZAgentConfig {
...
function enablePluginEntry(cfg: ZAgentConfig, pluginId: string): ZAgentConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
