...
import type { ZAgentConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ZAgentConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: ZAgentConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ZAgentSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ZAgentConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ZAgentConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as ZAgentConfig)),
    ),
  };
}
