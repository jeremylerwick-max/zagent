...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "zagent-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".zagent", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<ZAgentConfig> = {},
): ZAgentConfig {
  const base: ZAgentConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".zagent", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
