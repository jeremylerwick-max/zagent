...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ZAGENT_GATEWAY_PORT).toBe("18789");
    expect(env.ZAGENT_GATEWAY_TOKEN).toBe("secret");
    expect(env.ZAGENT_SERVICE_MARKER).toBe("zagent");
    expect(env.ZAGENT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ZAGENT_SERVICE_VERSION).toBe("string");
    expect(env.ZAGENT_SYSTEMD_UNIT).toBe("zagent-gateway.service");
    if (process.platform === "darwin") {
      expect(env.ZAGENT_LAUNCHD_LABEL).toBe("bot.molt.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ZAGENT_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ZAGENT_SYSTEMD_UNIT).toBe("zagent-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.ZAGENT_LAUNCHD_LABEL).toBe("bot.molt.work");
    }
  });
});
