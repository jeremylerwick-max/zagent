...
from db.evidence_repo import EvidenceRepo


async def db_create_job(state: dict, config: RunnableConfig) -> dict:
    deps: Deps = config["configurable"]["deps"]
    budgets: Dict[str, Any] = state.get("budgets", {}) or {}
    async with deps.sessionmaker() as session:
        repo = EvidenceRepo(session)
        job_id = await repo.create_job(state["user_query"], budgets=budgets, user_id=state.get("user_id"))
...
async def db_update_plan(state: dict, config: RunnableConfig) -> dict:
    ...
    async with deps.sessionmaker() as session:
        ...
        await repo.update_job_plan(
            ...
            stopping_criteria=state.get("stopping_criteria", {}),
        )
        await session.commit()
    return state
...
async def db_persist_extractions(state: dict, config: RunnableConfig) -> dict:
    ...
    if unsupported_all:
        ...
        gaps.append(f"{len(unsupported_all)} high-confidence claims lacked citations; added strict enforcement.")
        for t in unsupported_all[:5]:
            gaps.append(f"Need citations for claim: {t}")
        state["gaps"] = gaps
...
async def db_finalize(state: dict, config: RunnableConfig) -> dict:
    ...
    async with deps.sessionmaker() as session:
        ...
        await repo.finalize(
            job_id=state["job_id"], answer=answer, outline=outline,
