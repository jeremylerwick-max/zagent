...
import { resolveZAgentAgentDir } from "../agents/agent-paths.js";
...
import { ensureZAgentModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { ZAgentConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.ZAGENT_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.ZAGENT_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.ZAGENT_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<ZAgentConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.ZAGENT_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.ZAGENT_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveZAgentAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-live-state-"));
  process.env.ZAGENT_STATE_DIR = tempStateDir;
  ...
  process.env.ZAGENT_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.zagent-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveZAgentAgentDir();
  const sanitizedCfg: ZAgentConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "ZAgent live tool probe (local, safe): " +
    ...
    process.env.ZAGENT_CONFIG_PATH = previous.configPath;
    process.env.ZAGENT_GATEWAY_TOKEN = previous.token;
    process.env.ZAGENT_SKIP_CHANNELS = previous.skipChannels;
    process.env.ZAGENT_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.ZAGENT_SKIP_CRON = previous.skipCron;
    process.env.ZAGENT_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.ZAGENT_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.ZAGENT_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureZAgentModelsJson(cfg);
      ...
      const rawModels = process.env.ZAGENT_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.zagent-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
