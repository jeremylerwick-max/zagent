...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.ZAGENT_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-gw-mock-home-"));
      ...
      const toolProbePath = path.join(workspaceDir, `.zagent-tool-probe.${nonceA}.txt`);
      ...
      try {
        ...
        process.env.ZAGENT_CONFIG_PATH = prev.configPath;
        process.env.ZAGENT_GATEWAY_TOKEN = prev.token;
        process.env.ZAGENT_SKIP_CHANNELS = prev.skipChannels;
        process.env.ZAGENT_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.ZAGENT_SKIP_CRON = prev.skipCron;
        process.env.ZAGENT_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.ZAGENT_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.ZAGENT_STATE_DIR = prev.stateDir;
      process.env.ZAGENT_CONFIG_PATH = prev.configPath;
      process.env.ZAGENT_GATEWAY_TOKEN = prev.token;
      process.env.ZAGENT_SKIP_CHANNELS = prev.skipChannels;
      process.env.ZAGENT_SKIP_GMAIL_WATCHER = prev.skipGmail;
