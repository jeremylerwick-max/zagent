...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "ZAgent";
  if (/zagent/i.test(trimmed)) return trimmed;
  return `${trimmed} (ZAgent)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.ZAGENT_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "zagent");
  ...
  const binCli = path.join(cwd, "bin", "zagent.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.ZAGENT_TAILNET_DNS?.trim();
