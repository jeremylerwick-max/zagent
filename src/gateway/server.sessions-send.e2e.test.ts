...
import { createZAgentTools } from "../agents/zagent-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.ZAGENT_GATEWAY_PORT;
  prevGatewayToken = process.env.ZAGENT_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.ZAGENT_GATEWAY_PORT = String(gatewayPort);
  process.env.ZAGENT_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.ZAGENT_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.ZAGENT_GATEWAY_TOKEN;
  } else {
    process.env.ZAGENT_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createZAgentTools().find((candidate) => candidate.name === "sessions_send");
