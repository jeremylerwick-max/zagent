...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `zagent-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.ZAGENT_CONFIG_PATH = path.join(root, "zagent.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "zagent.json");
