...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-zagent-message-channel") ?? "");
  const accountId = getHeader(req, "x-zagent-account-id")?.trim() || undefined;
  ...
  const allTools = createZAgentTools({
