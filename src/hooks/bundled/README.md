...
This directory contains hooks that ship with ZAgent. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
metadata:
  { "zagent": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.zagent/zagent.json`:
...
2. Restart gateway: `pkill -9 -f 'zagent.*gateway' && pnpm zagent gateway`
3. Enable the hook: `zagent hooks enable my-hook`
