...
2. **Appends to log file** - Writes a JSON line to `~/.zagent/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.zagent/logs/commands.log ~/.zagent/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/zagent`:
   ```
   /home/username/.zagent/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
