...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ZAGENT_DISABLE_BONJOUR)) return true;
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "ZAgent";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(ZAgent\)\s*$/i, "").trim() || normalized;
}
