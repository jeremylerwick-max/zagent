...
function candidateBinDirs(opts: EnsureZAgentPathOpts): string[] {
  ...
  // Bundled macOS app: `zagent` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingZAgent = path.join(execDir, "zagent");
    if (isExecutable(siblingZAgent)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/zagent` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "zagent"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `zagent` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureZAgentCliOnPath(opts: EnsureZAgentPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ZAGENT_PATH_BOOTSTRAPPED)) return;
  process.env.ZAGENT_PATH_BOOTSTRAPPED = "1";
