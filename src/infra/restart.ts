...
export function triggerZAgentRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.ZAGENT_SYSTEMD_UNIT,
        process.env.ZAGENT_PROFILE,
      );
  ...
  const label =
    process.env.ZAGENT_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ZAGENT_PROFILE);
