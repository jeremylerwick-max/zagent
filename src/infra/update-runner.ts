...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "zagent doctor",
        managerScriptArgs(manager, "zagent", ["doctor", "--non-interactive"]),
        gitRoot,
        { ZAGENT_UPDATE_IN_PROGRESS: "1" },
      ),
    );
