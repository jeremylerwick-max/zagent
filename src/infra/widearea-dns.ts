...
export const WIDE_AREA_DISCOVERY_DOMAIN = "zagent.internal.";
export const WIDE_AREA_ZONE_FILENAME = "zagent.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*zagent-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "zagent";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "zagent");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "zagent-gw");
  ...
  records.push(`_zagent-gw._tcp IN PTR ${instanceLabel}._zagent-gw._tcp`);
  records.push(`${instanceLabel}._zagent-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._zagent-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; zagent-content-hash: ${contentHash}\n${contentBody}`;
}
