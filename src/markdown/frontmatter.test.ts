...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { zagent?: { emoji?: string } };
    expect(parsed.zagent?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { zagent?: { events?: string[] } };
