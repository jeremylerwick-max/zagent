...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "ZAGENT_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "zagent-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.ZAGENT_STATE_DIR = path.join(home, ".zagent");
    ...
    await fs.mkdir(path.join(home, ".zagent"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.zagent${path.sep}media`);
