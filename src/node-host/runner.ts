...
const execHostEnforced = process.env.ZAGENT_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.ZAGENT_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureZAgentCliOnPath({ pathEnv: process.env.PATH ?? "" });
