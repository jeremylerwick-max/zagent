...
import { loadZAgentPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.ZAGENT_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `zagent-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "zagent.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ZAGENT_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadZAgentPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.ZAGENT_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadZAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadZAgentPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
