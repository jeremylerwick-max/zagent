...
import { discoverZAgentPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ZAgentPluginDefinition;
  register?: ZAgentPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ZAgentPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ZAgentPluginDefinition;
...
export function loadZAgentPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverZAgentPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "zagent/plugin-sdk": pluginSdkAlias,
            "zagent/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ZAgentPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as ZAgentPluginModule;
    } catch (err) {
