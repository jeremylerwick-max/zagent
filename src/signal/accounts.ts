...
export function listSignalAccountIds(cfg: ZAgentConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: ZAgentConfig): string {
...
function mergeSignalAccountConfig(cfg: ZAgentConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: ZAgentConfig): ResolvedSignalAccount[] {
