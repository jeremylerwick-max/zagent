...
export function listSlackAccountIds(cfg: ZAgentConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: ZAgentConfig): string {
...
function mergeSlackAccountConfig(cfg: ZAgentConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: ZAgentConfig): ResolvedSlackAccount[] {
