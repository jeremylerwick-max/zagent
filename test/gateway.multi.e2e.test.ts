...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `zagent-e2e-${name}-`));
  const configDir = path.join(homeDir, ".zagent");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          ZAGENT_CONFIG_PATH: configPath,
          ZAGENT_STATE_DIR: stateDir,
          ZAGENT_GATEWAY_TOKEN: "",
          ZAGENT_GATEWAY_PASSWORD: "",
          ZAGENT_SKIP_CHANNELS: "1",
          ZAGENT_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        ZAGENT_GATEWAY_TOKEN: inst.gatewayToken,
        ZAGENT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ZAGENT_GATEWAY_PORT: String(gwA.port),
          ZAGENT_GATEWAY_TOKEN: gwA.gatewayToken,
          ZAGENT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          ZAGENT_GATEWAY_PORT: String(gwB.port),
          ZAGENT_GATEWAY_TOKEN: gwB.gatewayToken,
          ZAGENT_GATEWAY_PASSWORD: "",
        }),
