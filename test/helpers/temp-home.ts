...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("ZAGENT_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.ZAGENT_STATE_DIR = path.join(base, ".zagent");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "zagent-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".zagent", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
