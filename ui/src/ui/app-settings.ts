...
import type { ZAgentApp } from "./app";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as ZAgentApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as ZAgentApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as ZAgentApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as ZAgentApp);
    await loadDevices(host as unknown as ZAgentApp);
    await loadConfig(host as unknown as ZAgentApp);
    await loadExecApprovals(host as unknown as ZAgentApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as ZAgentApp);
    await loadConfig(host as unknown as ZAgentApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as ZAgentApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as ZAgentApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__ZAGENT_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ZAgentApp, false),
    loadPresence(host as unknown as ZAgentApp),
    loadSessions(host as unknown as ZAgentApp),
    loadCronStatus(host as unknown as ZAgentApp),
    loadDebug(host as unknown as ZAgentApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ZAgentApp, true),
    loadConfigSchema(host as unknown as ZAgentApp),
    loadConfig(host as unknown as ZAgentApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as ZAgentApp, false),
    loadCronStatus(host as unknown as ZAgentApp),
    loadCronJobs(host as unknown as ZAgentApp),
  ]);
}
