...
import { ZAgentApp } from "./app";
import "../styles.css";

const originalConnect = ZAgentApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("zagent-app") as ZAgentApp;
...
beforeEach(() => {
  ZAgentApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__ZAGENT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  ZAgentApp.prototype.connect = originalConnect;
  window.__ZAGENT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/zagent/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/zagent");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/zagent/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__ZAGENT_CONTROL_UI_BASE_PATH__ = "/zagent";
    const app = mountApp("/zagent/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/zagent");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/zagent/sessions");
  });
